...
struct Day14: Day {
    ...
    func run(input: String) async throws -> (Int, Int) {
        ...
        async let p1 = part1(reactions: reactions)
        async let p2 = part2(reactions: reactions)
        return try await (p1, p2)
    }

    func part1(reactions: [Reaction]) async throws -> Int {
        return requirements(for: ["FUEL": 1], reactions: reactions)
    }

    func part2(reactions: [Reaction]) async throws -> Int {
        var mostFuel = 0
