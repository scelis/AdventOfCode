...
struct Day10: Day {
    func parseAsteroids(input: String) -> Set<Coordinate2D> {
    ...
    func run(input: String) async throws -> (Int, Int) {
        let asteroids = parseAsteroids(input: input)
        async let p1 = part1(asteroids: asteroids)
        async let p2 = part2(asteroids: asteroids)
        return try await (p1, p2)
    }

    func part1(asteroids: Set<Coordinate2D>) async throws -> Int {
        var mostDetected = 0
    ...
    func part2(asteroids: Set<Coordinate2D>) async throws -> Int {
