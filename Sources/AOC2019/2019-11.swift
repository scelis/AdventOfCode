...
struct Day11: Day {
    ...
    func runComputer(input: String, startOnWhite: Bool = false) -> [Coordinate2D: Color] {
    ...
    func part1(input: String) async throws -> Int {
        let coordinatesPainted = runComputer(input: input)
        return coordinatesPainted.count
    }

    func part2(input: String) async throws -> String {
        let coordinatesPainted = runComputer(input: input, startOnWhite: true)
