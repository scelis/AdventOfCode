...
struct Day04: Day {
    ...
    func run(input: String) async throws -> (Int, Int) {
        ...
        let range = integers[0]...integers[1]
        async let p1 = part1(range: range)
        async let p2 = part2(range: range)
        return try await (p1, p2)
    }

    func part1(range: ClosedRange<Int>) async throws -> Int {
        return numberOfPasswords(in: range, strict: false)
    }

    func part2(range: ClosedRange<Int>) async throws -> Int {
