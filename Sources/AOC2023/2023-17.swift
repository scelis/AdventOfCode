...
struct Day17: Day {
    ...
    func run(input: String) async throws -> (Int, Int) {
        let map = parseMap(lines: input.lines)
        async let p1 = part1(map: map)
        async let p2 = part2(map: map)
        return try await (p1, p2)
    }

    func part1(map: [[Int]]) async throws -> Int {
        solve(map: map, minimumStepsBeforeTurning: 1, maximumStepsBeforeTurning: 3)
    }

    func part2(map: [[Int]]) async throws -> Int {
        solve(map: map, minimumStepsBeforeTurning: 4, maximumStepsBeforeTurning: 10)
    }

    func solve(map: [[Int]], minimumStepsBeforeTurning: Int, maximumStepsBeforeTurning: Int) -> Int {
