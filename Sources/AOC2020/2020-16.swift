...
struct Day16: Day {
    ...
    func scanTickets(lines: [String], fields: [String: IndexSet]) -> ScanTicketsResult {
        ...
        var errorRate = 0
        var potentialTickets: [[Int]] = []
        for ticket in lines.map({ $0.components(separatedBy: ",").map({ Int($0)! }) }) {
            var isValid = true
    ...
    func run(input: String) async throws -> (Int, Int) {
        ...
        let scanTicketsResult = scanTickets(lines: lines, fields: fields)
