...
struct Day17: Day {
    ...
    func run(input: String) async throws -> (String, Int) {
        let computer = parseInput(input: input)
        async let p1 = part1(computer: computer)
        async let p2 = part2(computer: computer)
        return try await (p1, p2)
    }

    func part1(computer: Computer) async throws -> String {
        var computer = computer
        computer.run()
        return computer.output.map(String.init).joined(separator: ",")
    }

    func part2(computer: Computer) async throws -> Int {
