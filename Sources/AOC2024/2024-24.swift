...
struct Day24: Day {
    ...
    func run(input: String) async throws -> (Int, String) {
        ...
        async let p1 = part1(wires: parsed.wires, gates: parsed.gates)
        async let p2 = part2(gates: parsed.gates)
        return try await (p1, p2)
    }

    func part1(wires: [String: Bool], gates: [Gate]) async throws -> Int {
    ...
    func part2(gates: [Gate]) async throws -> String {
        var gates = gates
        var outputs: [String] = []
    ...
    func parseInput(input: String) -> (wires: [String: Bool], gates: [Gate]) {
