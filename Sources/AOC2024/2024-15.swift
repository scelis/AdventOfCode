...
struct Day15: Day {
    ...
    func run(input: String) async throws -> (Int, Int) {
        async let p1 = solve(input: input, expanded: false)
        async let p2 = solve(input: input, expanded: true)
        return try await (p1, p2)
    }

    func solve(input: String, expanded: Bool) async throws -> Int {
        var (tiles, directions) = parseInput(input: input, expanded: expanded)
    ...
    func parseInput(input: String, expanded: Bool) -> ([[Tile]], [Direction]) {
