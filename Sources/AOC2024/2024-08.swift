...
struct Day08: Day {
    func run(input: String) async throws -> (Int, Int) {
        let (locations, xRange, yRange) = parseInput(input: input)
    ...
    func parseInput(input: String) -> ([Character: Set<Coordinate2D>], Range<Int>, Range<Int>) {
        let graph = input.characterArrays
